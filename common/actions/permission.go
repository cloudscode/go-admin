...
func PermissionUser(tableName string, p *DataPermission) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		...
		case "3":
			return db.Where(tableName+".user_id in (SELECT user_id from sys_user where dept_id = ? )", p.DeptId)
		case "4":
			return db.Where(tableName+".user_id in (SELECT user_id from sys_user where sys_user.dept_id in(select dept_id from sys_dept where dept_path like ? ))", "%/"+pkg.IntToString(p.DeptId)+"/%")
		//case "5":
		//	return db.Where(tableName+".create_by = ?", p.UserId)
		default             :
			return db
